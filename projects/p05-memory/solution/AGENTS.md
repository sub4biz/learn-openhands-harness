...
- `scripts/` — dev scripts (`npm run dev` spawns agent-server + automation backend via uvx)
- `DEVELOPMENT.md` — canonical dev setup guide

## Key config
- `VITE_BACKEND_BASE_URL` / `VITE_BACKEND_HOST` — control which backend the frontend talks to; set by the dev script launcher
